...
class WePay
	...
	STAGE_API_ENDPOINT = "://stage.wepayapi.com/v2"
	STAGE_UI_ENDPOINT = "://stage.wepay.com/v2"

	PRODUCTION_API_ENDPOINT = "://wepayapi.com/v2"
	PRODUCTION_UI_ENDPOINT = "://www.wepay.com/v2"
    ...
    protocol = _use_ssl ? 'https' : 'http'
		if _use_stage
			@api_endpoint = protocol + STAGE_API_ENDPOINT
			@ui_endpoint = protocol + STAGE_UI_ENDPOINT
		else
			@api_endpoint = protocol + PRODUCTION_API_ENDPOINT
			@ui_endpoint = protocol + PRODUCTION_UI_ENDPOINT
		end
		@use_ssl = _use_ssl
	end
